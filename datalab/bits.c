 ...
 *  Name: Weiyao Tang    Student ID:109366927
...
int satAdd(int x, int y) {
  ...
  sum=x+y;
  sign_sum=sum>>31;
  overflow=(sign_x^sign_sum)&(sign_y^sign_sum);
  max_val=((0x7f)<<24)|((0xff)<<16)|((0xff)<<8)|(0xff);
  return (overflow & (max_val+(!!sign_x)))| ((~overflow)&sum );
}
/*
 * Extra credit
 */
